var arr0 = [
    {lou: {
        firstname: 'Lou ',
        lastname: 'Schulte',
        name: 'Lou Schulte',
        homenum: '',
        busnum: '727-553-7836',
        ex: '32034',
        cellnum: '727-408-0858',
        email: 'Lou.Schulte@hma.com',
        company: ''
    }
        }, {
        perkeisha: {
            firstname: 'Perkeisha',
            lastname: 'Underwood',
            name: 'Perkeisha Underwood',
            homenum: '754-366-6908',
            busnum: '727-553-7834',
            ex: '37834',
            cellnum: '754-366-6908',
            email: 'Perkeisha.Underwood@hma.com',
            company: ''
        }
    }, {
        tom: {
            firstname: 'Tom ',
            lastname: 'Tindall',
            name: 'Tom Tindall',
            homenum: '727-823-5241',
            busnum: '727-553-7837',
            ex: '37837',
            cellnum: '941-224-5045',
            email: 'Tom.Tindall@hma.com',
            company: ''
        }
    }, {
        tammy: {
            firstname: 'Tammy ',
            lastname: 'Beal',
            name: 'Tammy Beal',
            homenum: '727-867-8213',
            busnum: '727-893-6238',
            ex: '32033',
            cellnum: '727-366-7370',
            email: 'tammy.beal@hma.com',
            company: ''
        }
    }, {
        colleen: {
            firstname: 'Colleen ',
            lastname: 'Hoffman',
            name: 'Colleen Hoffman',
            homenum: '609-694-8526',
            busnum: '727-553-7414',
            ex: '37414',
            cellnum: '609-694-8526',
            email: 'Colleen.Hoffman@hma.com',
            company: ''
        }
    }, {
        john: {
            firstname: 'John ',
            lastname: 'Amodeo',
            name: 'John Amodeo',
            homenum: '7278678213',
            busnum: '7278936691',
            ex: '36691',
            cellnum: '',
            email: 'John.Amodeo@hma.com',
            company: ''
        }
    }, {
        sharon: {
            firstname: 'Sharon ',
            lastname: 'Hodge',
            name: 'Sharon Hodge',
            homenum: '',
            busnum: '',
            cellnum: '',
            email: 'Sharon.Hodge@hma.com',
            company: ''
        }
    }, {
        denise: {
            firstname: 'Denise ',
            lastname: 'Rafferty',
            name: 'Denise Rafferty',
            homenum: '',
            busnum: '1-800-366-8020',
            cellnum: '1-423-243-6759',
            email: '',
            company: 'Abbott'
        }
    }, {
        sean: {
            firstname: 'Sean ',
            lastname: 'Fogarty',
            name: 'Sean Fogarty',
            homenum: ' ',
            busnum: '727-893-6494',
            ex: '36182',
            cellnum: '727-408-4330',
            email: 'Sean.Fogarty@hma.com',
            company: ''
        }
    }, {
        shari: {
            firstname: 'Shari ',
            lastname: 'Goodwin',
            name: 'Shari Goodwin',
            homenum: '727-531-7655',
            busnum: '727-553-7820',
            ex: '37820',
            cellnum: '727-560-8931',
            email: 'Shari.Goodwin@hma.com',
            company: ''
        }
    }, {
        timothy: {
            firstname: 'Timothy ',
            lastname: 'Luba',
            name: 'Timothy Luba',
            homenum: '',
            busnum: '727-893-6160',
            ex: '33142',
            cellnum: '727-224-1136',
            email: 'Timothy.Luba@hma.com',
            company: ''
        }
    }, {
        deniseM: {
            firstname: 'Denise ',
            lastname: 'Murphy',
            name: 'Denise Murphy',
            homenum: '727-323-3631',
            busnum: '727-553-7838',
            ex: '37838',
            cellnum: '727-218-3825',
            email: 'Denise.Murphy@hma.com',
            company: ''
        }
    }, {
        nadena: {
            firstname: 'Nadena ',
            lastname: 'Martinovich',
            name: 'Nadena Martinovich',
            homenum: '',
            busnum: '7278936028',
            ex: '',
            cellnum: '',
            email: 'Nadena.Martinovich@hma.com',
            company: ''
        }
    }, {
        chirayu: {
            firstname: 'Chirayu ',
            lastname: 'Patel',
            name: 'Chirayu Patel',
            homenum: '',
            busnum: '',
            cellnum: '',
            email: '',
            company: ''
        }
    }, {
        stephen: {
            firstname: 'Stephen ',
            lastname: 'Marts',
            name: 'Stephen Marts',
            homenum: '',
            busnum: '727-893-6342',
            ex: '36342',
            cellnum: '816-797-1755',
            email: 'Stephen.Marts@hma.com',
            company: ''
        }
    }, {
        rick: {
            firstname: 'Rick ',
            lastname: 'Gibner',
            name: 'Rick Gibner',
            homenum: '813-633-7556',
            busnum: '727-553-7898',
            ex: '37898',
            cellnum: '',
            email: 'Rick.Gibner@hma.com',
            company: ''
        }
    }, {
        dave: {
            firstname: 'Dave ',
            lastname: 'Leclaire',
            name: 'Dave Leclaire',
            homenum: '941-776-8385',
            busnum: '727-893-6034',
            ex: '32029',
            cellnum: '727-710-9937',
            email: 'Dave.Leclaire@hma.com',
            company: ''
        }
    }, {
        maryBeth: {
            firstname: 'Mary Beth ',
            lastname: 'Moody',
            name: 'MaryBeth Moody',
            homenum: '',
            busnum: '727-893-6138',
            ex: '36138',
            cellnum: '',
            email: 'MaryBeth.Moody@hma.com',
            company: ''
        }
    }, {
        aston: {
            firstname: 'Aston ',
            lastname: 'Samuels',
            name: 'Aston Samuels',
            homenum: '561-449-1333',
            busnum: '727-290-1362',
            ex: '32025',
            cellnum: '561-449-1333',
            email: 'Aston.Samuels@hma.com',
            company: ''
        }
    }, {
        shannon: {
            firstname: 'Shannon ',
            lastname: 'Pauley',
            name: 'Shannon Pauley',
            homenum: '',
            busnum: '',
            cellnum: '',
            email: '',
            company: ''
        }
    }, {
        daniel: {
            firstname: 'Daniel ',
            lastname: 'Downs',
            name: 'Daniel Downs',
            homenum: '727-612-8204',
            busnum: '727-896-6348',
            ex: '36348',
            cellnum: '',
            email: 'Daniel.Downs@hma.com',
            company: ''
        }
    }, {
        johnG: {
            firstname: 'John ',
            lastname: 'Gaskins',
            name: 'John Gaskins',
            homenum: '',
            busnum: '',
            ex: '52011',
            cellnum: '727-504-9376',
            email: 'John.Gaskins@hma.com',
            company: ''
        }
    }, {
        tyler: {
            firstname: 'Tyler ',
            lastname: 'Betts',
            name: 'Tyler Betts',
            homenum: '239-565-0045',
            busnum: '727-893-7717',
            ex: '37717',
            cellnum: '239-565-0045',
            email: 'Tyler.Betts@hma.com',
            company: ''
        }
    }, {
        bob: {
            firstname: 'Bob ',
            lastname: 'Fox',
            name: 'Bob Fox',
            homenum: '',
            busnum: '727-893-6147',
            ex: '36147',
            cellnum: '727-519-3574',
            email: 'Robert.Fox@hma.com',
            company: ''
        }
    }, {
        jeremy: {
            firstname: 'Jeremy ',
            lastname: 'Dean',
            name: 'Jeremy Dean',
            homenum: '',
            busnum: '',
            ex: '',
            cellnum: '',
            email: 'Robert.Fox@hma.com',
            company: ''
        }
    }, {
        lori: {
            firstname: 'Lori ',
            lastname: 'Bailey',
            name: 'Lori Bailey',
            homenum: '',
            busnum: '7278936268',
            ex: '36268',
            cellnum: '',
            email: 'Lori.Bailey@hma.com',
            company: ''
        }
    }, {
        amy: {
            firstname: 'Amy ',
            lastname: 'Kendricks',
            name: 'Amy Kendricks',
            homenum: '',
            busnum: '',
            ex: '',
            cellnum: '',
            email: 'Amy.Kendricks@hma.com',
            company: ''
        }
    }, {
        janet: {
            firstname: 'Janet ',
            lastname: 'Bryant',
            name: 'Janet Bryant',
            homenum: '',
            busnum: '',
            ex: '',
            cellnum: '',
            email: 'Janet.Bryant@hma.com',
            company: ''
        }
    }, {
        herman: {
            firstname: 'Herman ',
            lastname: 'Gilyard',
            name: 'Herman Gilyard',
            homenum: '813-442-8336',
            busnum: '727-893-6018',
            ex: '36018',
            cellnum: '727-410-6319',
            email: 'Herman.Gilyard@hma.com',
            company: ''
        }
    }, {
        mark: {
            firstname: 'Mark ',
            lastname: 'Smithson',
            name: 'Mark Smithson',
            homenum: '727-523-1292',
            busnum: '727-893-6730',
            ex: '36730',
            cellnum: '727-481-3233',
            email: 'Mark.Smithson@hma.com',
            company: ''
        }
    }, {
        barbara: {
            firstname: 'Barbara ',
            lastname: 'Robinson',
            name: 'Barbara Robinson',
            homenum: '813-661-3184',
            busnum: '727-893-6147',
            ex: '36147',
            cellnum: '727-481-3233',
            email: 'Barbara.Robinson@hma.com',
            company: ''
        }
    }, {
        tony: {
            firstname: 'Tony ',
            lastname: 'Ruiz',
            name: 'Tony Ruiz',
            homenum: '813-884-7271',
            busnum: '727-893-7441',
            ex: '37441',
            cellnum: '813-502-9504',
            email: 'anthony.ruiz@hma.com',
            company: ''
        }
    }, {
        ernest: {
            firstname: 'Ernest ',
            lastname: 'Whalen',
            name: 'Ernest Whalen',
            homenum: '727-798-1911',
            busnum: '727-893-7441',
            ex: '37441',
            cellnum: '727-798-1911',
            email: 'Ernest.Whalen@hma.com',
            company: ''
        }
    }, {
        morgan: {
            firstname: 'Morgan ',
            lastname: 'Harris',
            name: 'Morgan Harris',
            homenum: '',
            busnum: '727-893-7441',
            ex: '37441',
            cellnum: '727-798-1911',
            email: 'Morgan.Harris@hma.com',
            company: ''
        }
    }, {
        ignacio: {
            firstname: 'Ignacio ',
            lastname: 'Fernandez',
            name: 'Ignacio Fernandez',
            homenum: '813-933-1040',
            busnum: '727-893-7117',
            ex: '37117',
            cellnum: '813-933-1020',
            email: 'Ignacio.Fernandez@hma.com',
            company: ''
        }
    }, {
        patti: {
            firstname: 'Patti ',
            lastname: 'Ewin',
            name: 'Patti Ewin',
            homenum: '',
            busnum: '727-893-6351',
            ex: '36351',
            cellnum: '',
            email: 'Patti.Ewin@hma.com',
            company: ''
        }
    }, {
        cecilio: {
            firstname: 'Cecilio ',
            lastname: 'Shaw',
            name: 'Cecilio Shaw',
            homenum: '813-956-6954',
            busnum: '727-893-6776',
            ex: '36776',
            cellnum: '813-956-6954',
            email: 'Cecilio.Shaw@hma.com',
            company: ''
        }
    }, {
        rasheedah: {
            firstname: 'Rasheedah ',
            lastname: 'Haneef',
            name: 'Rasheedah Haneef',
            homenum: '',
            busnum: '727-893-6613',
            ex: '36613',
            cellnum: '',
            email: 'Rasheedah.Haneef@hma.com',
            company: ''
        }
    }, {
        edwin: {
            firstname: 'Edwin ',
            lastname: 'Rivas',
            name: 'Edwin Rivas',
            homenum: '813-740-2500',
            busnum: '800-752-7725',
            ex: '36613',
            cellnum: '',
            email: '',
            company: 'Rauland Borg'
        }
    }, {
        lauren: {
            firstname: 'Lauren ',
            lastname: 'Hinds',
            name: 'Lauren Hinds',
            homenum: '813-740-2500',
            busnum: '800-752-7725',
            ex: '36613',
            cellnum: '',
            email: '',
            company: 'Rauland Borg'
        }
    }, {
        peter: {
            firstname: 'Peter ',
            lastname: 'Napoles',
            name: 'Peter Napoles',
            homenum: '',
            busnum: '727-893-7112',
            ex: '37112',
            cellnum: '305-798-2070',
            email: 'Peter.Napoles@hma.com',
            company: ''
        }
    }, {
        bernard: {
            firstname: 'Bernard ',
            lastname: 'Butler',
            name: 'Bernard Butler',
            homenum: '727-867-4069',
            busnum: '727-893-6908',
            ex: '32011',
            cellnum: '727-403-1206',
            email: 'Bernard.Butler@hma.com',
            company: ''
        }
    }, {
        kenn: {
            firstname: 'Kenn ',
            lastname: 'Finnis',
            name: 'Kenn Finnis',
            homenum: '727-710-3664',
            busnum: '727-553-7444',
            ex: '32022',
            cellnum: '727-710-3664',
            email: 'Kenn.Finnis@hma.com',
            company: ''
        }
    }, {
        biomed: {
            firstname: 'biomed ',
            lastname: '',
            name: 'biomed',
            homenum: '',
            busnum: '727-257-0165',
            ex: '36904',
            cellnum: '',
            email: '',
            company: ''
        }
    }, {
        jorge: {
            firstname: 'Jorge ',
            lastname: 'Gimenez',
            name: 'Jorge Gimenez',
            homenum: '',
            busnum: '727-893-6569',
            ex: '36569',
            cellnum: '727-366-9779',
            email: 'Jorge.Gimenez@hma.com',
            company: ''
        }
    }, {
        brooke: {
            firstname: 'Brooke ',
            lastname: 'Gabel',
            name: 'Brooke Gabel',
            homenum: '',
            busnum: '',
            ex: '33355',
            cellnum: '',
            email: 'Brooke.Gabel@hma.com',
            company: ''
        }
    }
];

var arr =    [{
        appl: 'aast grand rounds',
        anal1: arr0[1].perkeisha
    },
    
    
        {
            appl: 'aast',
            anal1: arr0[1].perkeisha
        },
       
           
        {
            appl: 'accent health',
            anal1: arr0[1].perkeisha
        },
        {
            appl: 'accudose carts and support stations',
            anal1: arr0[2].tom
        },
        {
            appl: 'accudose',
            anal1: arr0[2].tom
        },
        {
            appl: 'ad/cerner account creation',
            anal1: arr0[3].tammy
                
        },
        {
            appl: 'ad',
            anal1: arr0[3].tammy
        },
        {
            appl: 'aegis',
            anal1: arr0[4].colleen
        },
        {
            appl: 'allegiantmd radiology',
            anal1: arr0[4].colleen
        },
        {
            appl: 'analyze direct',
            anal1: arr0[4].colleen
        },
        {
            appl: 'ansos',
            anal1: arr0[5].john
        },
        {
            appl: 'ariba',
            anal1: arr0[6].sharon
        },
        {
            appl: 'cerner',
            anal1: arr0[4].colleen
        
        },
        {
            appl: 'cerner (point of care) abbott',
            anal1: arr0[7].denise,
            anal2: arr0[8].sean,
            anal3: arr0[9].shari
        },
        {
            appl: 'cerner (point of care)- blood gases- telcor/quiklinc/radiance (radiometer)',
            anal1: arr0[9].shari,
            anal2: arr0[7].denise,
            anal3: arr0[10].timothy
        },
        {
            appl: 'cerner (point of care)- istat-cds-telcor/quiklinc',
            anal1: arr0[7].denise,
            anal2: arr0[8].sean,
            anal3: arr0[9].shari
        },
        {
            appl: 'cerner access',
            anal1: arr0[11].deniseM,
            anal2: arr0[0].lou
        },
        {
            appl: 'cerner antibiotic alert',
            anal1: arr0[9].shari
        },
        {
            appl: 'cerner blood bank',
            anal1: arr0[7].denise,
            anal2: arr0[7].nadena
        },
        {
            appl: 'cerner carecompass',
            anal1: arr0[13].chirayu
        },
        {
            appl: 'cerner cpoe',
            anal1: arr0[11].deniseM
        },
        {
            appl: 'cerner document imaging (cpdi- single/batch scanning)',
            anal1: arr0[14].stephen,
            anal2: arr0[2].tom
        },
        {
            appl: 'cerner document imaging (poi- pharmacy order imaging)',
            anal1: arr0[1].perkeisha
        },
        {
            appl: 'cerner firstnet',
            anal1: arr0[5].john
        },
        {
            appl: 'cerner health sentry',
            anal1: arr0[9].shari
        },
        {
            appl: 'cerner hub',
            anal1: arr0[15].rick
        },
        {
            appl: 'cerner inet downloads- spacelabs ics / g2 / clinical access',
            anal1: arr0[9].shari
        },
        {
            appl: 'cerner iview',
            anal1: arr0[13].chirayu
        },
        {
            appl: 'cerner message center/inbox',
            anal1: arr0[7].denise
        },
        {
            appl: 'cerner mpages',
            anal1: arr0[13].chirayu
        },
        {
            appl: 'cerner mrp medical records publishing (clinical reporting xr)',
            anal1: arr0[9].shari
        },
        {
            appl: 'cerner open engine interfaces- fsi',
            anal1: arr0[15].rick
        },
        {
            appl: 'cerner open port interfaces- fsi',
            anal1: arr0[15].rick
        },
        {
            appl: 'cerner pacs (picture archical clinical system',
            anal1: arr0[4].colleen
        },
        {
            appl: 'cerner pathnet (anatomic pathology, gen lab, microbiology)',
            anal1: arr0[7].denise,
            anal2: arr0[9].shari
        },
        {
            appl: 'cerner pathnet interfaces (mdi) cas11 and cas12',
            anal1: arr0[7].denise,
            anal2: arr0[9].shari,
            anal3: arr0[9].dave
        },
        {
            appl: 'cerner pathnet interfaces (mdi)- direct ip-  genexpert- microbiology (not live yet)',
            anal1: arr0[9].dave,
            anal2: arr0[7].denise,
            anal3: arr0[7].maryBeth,
            anal4: arr0[9].shari
        },
        {
            appl: 'cerner pathnet interfaces (mdi)- direct ip-  iris- jresultnet light urinalysis',
            anal1: arr0[9].dave,
            anal2: arr0[7].denise,
            anal3: arr0[9].shari
        },
        {
            appl: 'cerner patient depart process',
            anal1: arr0[5].john
        },
        {
            appl: 'cerner patient portal',
            anal1: arr0[1].perkeisha
        },
        {
            appl: 'cerner pharmnet',
            anal1: arr0[2].tom
        },
        {
            appl: 'cerner powerchart',
            anal1: arr0[4].colleen,
            anal2: arr0[18].aston,
            anal3: arr0[9].shari
        },
        {
            appl: 'cerner profile',
            anal1: arr0[14].stephen
        },
        {
            appl: 'cerner profit',
            anal1: arr0[14].shannon
        },
        {
            appl: 'cerner radnet',
            anal1: arr0[4].colleen
        },
        {
            appl: 'cerner scheduling',
            anal1: arr0[14].stephen
        },
        {
            appl: 'cerner surginet',
            anal1: arr0[5].john
        },
           
        {
            appl: 'dar',
            anal1: arr0[0].lou
        },
        
        {
            appl: 'desktop support',
            anal1: arr0[20].daniel,
            anal2: arr0[21].johnG,
            anal3: arr0[22].tyler
        },
        {
            appl: 'dexter cedaron',
            anal1: arr0[4].colleen
        },
        {
            appl: 'dragon',
            anal1: arr0[18].aston,
            anal2: arr0[4].colleen
        },
        {
            appl: 'echo system- former med staff credentialing system',
            anal1: arr0[11].deniseM,
            anal2: arr0[11].lori
        },
        {
            appl: 'eeg xltek',
            anal1: arr0[4].colleen
        },
        {
            appl: 'elsevier',
            anal1: arr0[26].amy
        },
        {
            appl: 'ers (event reporting system)',
            anal1: arr0[15].rick
        },
        {
            appl: 'excelera',
            anal1: arr0[4].colleen
        },
        {
            appl: 'fl ens (events notification system) interface',
            anal1: arr0[15].rick
        },
        {
            appl: 'fl newborn tiff alternative printing',
            anal1: arr0[9].shari
        },
        {
            appl: 'forms on demand (fod)',
            anal1: arr0[1].perkeisha
        },
        {
            appl: 'ge centricity cardiovascular  (321 206 8419)  vs. philips cardiovascular',
            anal1: arr0[4].colleen
        },
        {
            appl: 'ge fetal monitoring qs',
            anal1: arr0[4].colleen
        },
              {
            appl: 'ge muse',
            anal1: arr0[4].colleen
        },
        {
            appl: 'golden hour (bayflite)',
            anal1: arr0[7].denise
        },
        {
            appl: 'ibm iseries',
            anal1: arr0[0].lou
        },
        {
            appl: 'idinc ems risk management software / employee injury / peer review',
            anal1: arr0[27].janet
        },
        {
            appl: 'fl ens (events notification system) interface',
            anal1: arr0[15].rick
        },
        {
            appl: 'fl newborn tiff alternative printing',
            anal1: arr0[9].shari
        },
        {
            appl: 'forms on demand (fod)',
            anal1: arr0[1].perkeisha
        },
        {
            appl: 'ge centricity cardiovascular  (321 206 8419)  vs. philips cardiovascular',
            anal1: arr0[4].colleen
        },
        {
            appl: 'ge fetal monitoring qs',
            anal1: arr0[4].colleen
        },
        {
            appl: 'ge muse',
            anal1: arr0[15].rick
        },
        {
            appl: 'infinium',
            anal1: arr0[0].lou
        },
        {
            appl: 'integrated informatics / old mars system',
            anal1: arr0[4].colleen
        },
        {
            appl: 'iris instrument lab',
            anal1: arr0[9].shari
        },
        {
            appl: 'it manager',
            anal1: arr0[28].herman,
            anal2: arr0[29].mark
        },
        {
            appl: 'it operator',
            anal1: arr0[23].bob,
            anal2: arr0[30].barbara,
            anal3: arr0[31].tony,
            anal4: arr0[32].ernest,
            anal5: arr0[33].morgan
        },
        {
            appl: 'kronos time and attendance',
            anal1: arr0[0].lou
        },
        {
            appl: 'labor lytics',
            anal1: arr0[0].lou
        },
        {
            appl: 'landmark transcription',
            anal1: arr0[14].stephen
        },
        {
            appl: 'mckesson cerme and interqual',
            anal1: arr0[14].stephen
        },
        {
            appl: 'meadows business systems',
            anal1: arr0[4].colleen
        },
        {
            appl: 'mobile pagers',
            anal1: arr0[34].ignacio
        },
        {
            appl: 'monarch',
            anal1: arr0[35].patti
        },
        {
            appl: 'nortel telecom',
            anal1: arr0[34].ignacio,
            anal2: arr0[36].cecilio
        },
        {
            appl: 'nuance powerscribe',
            anal1: arr0[4].colleen
        },
        {
            appl: 'passport - intellisource',
            anal1: arr0[14].stephen
        },
        {
            appl: 'passport onesource',
            anal1: arr0[37].rasheedah
        },
        {
            appl: 'pulse',
            anal1: arr0[0].lou
        },
        {
            appl: 'remel ids database - to lookup organism identification',
            anal1: arr0[9].shari
        },
        {
            appl: 'responder 5',
            anal1: arr0[38].edwin,
            anal2: arr0[39].lauren
        },
        {
            appl: 'report request prod_bay',
            anal1: arr0[9].shari
        },
        {
            appl: 'security cameras',
            anal1: arr0[40].peter
        },
        {
            appl: 'single sign on paradigm (sso)',
            anal1: arr0[41].bernard
        },
        {
            appl: 'sophos',
            anal1: arr0[42].kenn
        },
        {
            appl: 'spacelabs',
            anal1: arr0[43].biomed
        },
        {
            appl: 'specialty/ quest- see quest/specialty',
            anal1: arr0[9].shari
        },
        {
            appl: 'standard registry (forms on demand)',
            anal1: arr0[1].perkeisha
        },
        {
            appl: 'stryker',
            anal1: arr0[43].biomed
        },
        {
            appl: 'telecom',
            anal1: arr0[36].cecilio,
            anal2: arr0[34].ignacio
        },
        {
            appl: 'theradoc (infection control)',
            anal1: arr0[45].brooke
        },
        {
            appl: 'pharmacy',
            anal1: arr0[45].brooke
        },
        {
            appl: 'trauma base',
            anal1: arr0[5].john
        },
        {
            appl: 'truven mmd',
            anal1: arr0[1].perkeisha
        },
        {
            appl: 'vitrea',
            anal1: arr0[4].colleen
        },
        {
            appl: 'vocera',
            anal1: arr0[41].bernard,
            anal2: arr0[44].jorge
        },
        {
            appl: 'vrad',
            anal1: arr0[4].colleen
        },
        {
            appl: 'xcelera philips',
            anal1: arr0[4].colleen
        },
        {
            appl: 'xper philips (witt)',
            anal1: arr0[4].colleen
        },
        {
            appl: 'abbott',
            anal1: arr0[7].denise,
            anal2: arr0[8].sean,
            anal3: arr0[9].shari
        },
        {
            appl: 'grand rounds',
            anal1: arr0[1].perkeisha
        }
            ];




var app, arrLength = arr.length;



function myFunction() {
    

    
    var x, analystSupport, app;
    
    
    // Get the value of the input field with id="app"
    x = document.getElementById("app").value;
    app = x.toLowerCase();

    
    
    for (var i = 0; i < arrLength; i++){
if (app === arr[i].appl) {
    
    var anal1, anal2, anal3, anal4, anal5;
    
    console.log(arr[i].anal1.firstname);
    console.log(arr[i].anal1.lastname);
    
        analy1 = arr[i].anal1;
        analy2 = arr[i].anal2;
        analy3 = arr[i].anal3;
        analy4 = arr[i].anal4;
        analy5 = arr[i].anal5;
    
    if ((analy1 !== undefined) && (analy2 !== undefined) && (analy3 === undefined) && (analy4 === undefined) && (analy5 === undefined)){
                        
                document.getElementById("glyphu").src= "glyph/glyphicons-4-user.png";
                document.getElementById("name").innerHTML = analy1.name;
                document.getElementById("homenum").innerHTML = analy1.homenum;
                document.getElementById("busnum").innerHTML = analy1.busnum;
                document.getElementById("ex").innerHTML = analy1.ex;
                document.getElementById("cellnum").innerHTML = analy1.cellnum;
                document.getElementById("glyphe").src= "glyph/glyphicons-11-envelope.png";
                document.getElementById("email").innerHTML = analy1.email;
                document.getElementById("company").innerHTML = analy1.company;
                document.getElementById("glyphu").src= "glyph/glyphicons-4-user.png";
                document.getElementById("name2").innerHTML = analy2.name;
                document.getElementById("homenum2").innerHTML = analy2.homenum;
                document.getElementById("busnum2").innerHTML = analy2.busnum;
                document.getElementById("ex2").innerHTML = analy2.ex;
                document.getElementById("cellnum2").innerHTML = analy2.cellnum;
                document.getElementById("email2").innerHTML = analy2.email;
                document.getElementById("company2").innerHTML = analy2.company;
                document.getElementById("glyphu").src= "glyph/glyphicons-4-user.png";
                document.getElementById("name3").innerHTML = analy3.name;
                document.getElementById("homenum3").innerHTML = analy3.homenum;
                document.getElementById("busnum3").innerHTML = analy3.busnum;
                document.getElementById("ex3").innerHTML = analy3.ex;
                document.getElementById("cellnum3").innerHTML = analy3.cellnum;
                document.getElementById("email3").innerHTML = analy3.email;
                document.getElementById("company3").innerHTML = analy3.company;
                document.getElementById("glyphu").src= "glyph/glyphicons-4-user.png";
                document.getElementById("name4").innerHTML = analy4.name;
                document.getElementById("homenum4").innerHTML = analy4.homenum;
                document.getElementById("busnum4").innerHTML = analy4.busnum;
                document.getElementById("ex4").innerHTML = analy4.ex;
                document.getElementById("cellnum4").innerHTML = analy4.cellnum;
                document.getElementById("email4").innerHTML = analy4.email;
                document.getElementById("company4").innerHTML = analy4.company;
                document.getElementById("glyphu").src= "glyph/glyphicons-4-user.png";
                document.getElementById("name5").innerHTML = analy5.name;
                document.getElementById("homenum5").innerHTML = analy5.homenum;
                document.getElementById("busnum5").innerHTML = analy5.busnum;
                document.getElementById("ex5").innerHTML = analy5.ex;
                document.getElementById("cellnum5").innerHTML = analy5.cellnum;
                document.getElementById("email5").innerHTML = analy5.email;
                document.getElementById("company5").innerHTML = analy5.company;
    
    }

}
} //end of iterate through 'arr' array
    
}

            

function enterKey(e){
var key=e.keyCode || e.which;
if (key==13){
    myFunction();
}
}

